...
One gotcha with the bun install route: you don't get shell completions. Homebrew's formula generates them automatically, but it also pulls in node as a dependency, which defeats the purpose if you're using fnm to manage node versions. To get zsh completions with the bun install, generate them yourself:

```bash
nr --completion-zsh > $(brew --prefix)/share/zsh/site-functions/_nr
ni --completion-zsh > $(brew --prefix)/share/zsh/site-functions/_ni
```

Open a new terminal and `nr<tab>` will autocomplete your package.json scripts.
