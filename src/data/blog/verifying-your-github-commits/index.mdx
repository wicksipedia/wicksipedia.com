...
Have you ever looked a PR in GitHub and noticed that some commits have a _Verified_ chip and some don't? And that it's usually the PR merges that do? What gives?
...
2. I didn't like the sound of _GPG Suite_, so I went for Pinetry for Mac to store the passphrase of the PGP key in the OS keychain
...
6. Export and copy the key (make sure to include the _-----BEGIN PGP PUBLIC KEY BLOCK-----_ and _-----END PGP PUBLIC KEY BLOCK-----_)
...
7. Go to [Github - SSH and GPG Keys](https://github.com/settings/keys) and
   1. Click _New GPG Key_
   2. Paste the key in the textbox
   3. Click _Add GPG Key_
